<script>
    ...
    let nonSelectedPlayers = $state(players.filter(([player]) => 
    !selected.some(([selectedPlayer]) => selectedPlayer.name === player.name)))
</script>
...
{#if players.length}
    <section>
        ...
        <div class="space-y-2">
            ...
            {#each selected as [player, statistics] (player.id)}
                <div class="flex items-center p-3 rounded-lg {getBackgroundColor(position)} hover:shadow-md transition-shadow group relative border-2 border-yellow-300">
            ...
            <!-- Display non-selected players next -->
            {#each nonSelectedPlayers as [player, statistics] (player.id)}
