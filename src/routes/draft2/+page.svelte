<script>
    ...
    import { teams, playerTeam, draft, processedPlayers, gate0, gate1, draftOrderList } from "$lib/teams.svelte";
    import { 
    ...
    let pPlayers = $state(processedPlayers);
    let g0 = $state(gate0);
    let g1 = $state(gate1);
    let dOrderList = $state(draftOrderList);
    ...
    function beginDraft() {
        if (!draftState.started) {
            ...
            let currPick = dOrderList[0];
            let nextPick = dOrderList[1];
    ...
    function handleAIPick(teamId) {
        const result = executePick(teamId, false, playerTeam, teams, pPlayers);
        if (result) {
            pPlayers = result.processedPlayers;
            advanceDraft(draftState, dOrderList, playerTeam);
        }
    }
    
    function handlePlayerPick(player, statistics, transferValue) {
        const result = executePick('player', true, playerTeam, teams, pPlayers, player, statistics, transferValue);
        if (result) {
            pPlayers = result.processedPlayers;
            advanceDraft(draftState, dOrderList, playerTeam);
        }
    ...
    function createTeams() {
        ...
        generateClubTraits();
        dOrderList = organizeDraftOrder(playerTeam, teams);
        g1 = true;
    ...
    async function fetchAndProcessPlayers() {
        ...
        try {
            ...
            pPlayers = tempContainer.sort((a, b) => b.transferValue - a.transferValue);
            complete = true;
            g0 = true;
        } catch (err) {
    ...
    {#if pPlayers.length > 0}
            ...
            <div class="draft-ticker-container">
                ...
                {#if draftState.currentTeam !== playerTeam.name}
                        <div class="draft-buttons">
                            <button 
                                onclick={() => handleAIPick(dOrderList[(draftState.currentRound - 1) * 14 + (draftState.currentPick - 1)].id)} 
                                class="advance-btn">Advance Draft
                            </button>
                            <button 
                                onclick={() => {
                                    pPlayers = skipToPlayerPick(draftState, dOrderList, playerTeam, teams, pPlayers);
                                }} 
        ...
        <div class="page-container">
            <div class="players-section">
                <h3>Premier League Players by Transfer Value ({pPlayers.length})</h3>
                {#each pPlayers as {player, statistics, transferValue}}
                    <Player4 
                        player={player}
